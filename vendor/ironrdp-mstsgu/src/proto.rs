...
use ironrdp_core::{
    Decode, Encode, ReadCursor, WriteCursor, cast_int, cast_length, ensure_fixed_part_size,
...
impl HandshakeRespPkt {
    const FIXED_PART_SIZE: usize = 4 /* error_code */ + 1 /* ver_major */ + 1 /* ver_minor */ + 2 /* server_auth */ + 2 /*extended_auth*/;
}
...
impl Encode for TunnelReqPkt {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> ironrdp_core::EncodeResult<()> {
        ...
        // MS-TSGU 2.2.5.3.6: bit 0 is PAA_COOKIE, bit 1 is REAUTH.
        dst.write_u16(u16::from(self.paa_cookie.is_some()));
        dst.write_u16(self._reserved);
        if let Some(cookie) = &self.paa_cookie {
            dst.write_u16(cast_int!("PAA cookie length", cookie.len())?);
    ...
    fn size(&self) -> usize {
        PktHdr::default().size() + 8 + self.paa_cookie.as_ref().map_or(0, |cookie| 2 + cookie.len())
    }
}
...
impl Encode for TunnelAuthPkt {
    ...
    fn size(&self) -> usize {
        PktHdr::default().size() + 4 + 2 * (self.client_name.encode_utf16().count() + 1)
    }
}
...
mod relayne_tests {
    use super::*;
    #[test]
    fn paa_tunnel_cookie_field_has_exact_protocol_length_and_flag() {
        let packet = TunnelReqPkt { caps: 12, paa_cookie: Some(vec![0x41, 0, 0, 0]), ..Default::default() };
        let mut bytes = vec![0; packet.size()];
        packet.encode(&mut WriteCursor::new(&mut bytes)).unwrap();
        assert_eq!(bytes.len(), 22);
        assert_eq!(&bytes[4..8], &22u32.to_le_bytes());
        assert_eq!(&bytes[12..16], &[1, 0, 0, 0]);
        assert_eq!(&bytes[16..], &[4, 0, 0x41, 0, 0, 0]);
        let basic = TunnelReqPkt::default();
        assert_eq!(basic.size(), 16);
    }
    #[test]
    fn tunnel_client_name_unicode_has_no_trailing_padding() {
        let packet = TunnelAuthPkt { fields_present: 0, client_name: "東京😀".into() };
        let mut bytes = vec![0; packet.size()];
        let mut cursor = WriteCursor::new(&mut bytes);
        packet.encode(&mut cursor).unwrap();
        assert_eq!(cursor.pos(), packet.size());
        assert_eq!(packet.size(), 22);
    }
    #[test]
    fn extended_auth_roundtrip_and_truncation() {
        let packet = ExtendedAuthPkt {
            error_code: 0,
            blob: b"NTLMSSP\0test".to_vec(),
        };
        let mut bytes = vec![0; packet.size()];
        packet.encode(&mut WriteCursor::new(&mut bytes)).unwrap();
        let mut reader = ReadCursor::new(&bytes);
        let header = PktHdr::decode(&mut reader).unwrap();
        assert_eq!(header.ty, PktTy::ExtendedAuth);
        let decoded = ExtendedAuthPkt::decode(&mut reader).unwrap();
        assert_eq!(decoded.blob, packet.blob);
        assert!(ExtendedAuthPkt::decode(&mut ReadCursor::new(&bytes[8..bytes.len() - 1])).is_err());
    }
    #[test]
    fn truncated_handshake_is_error_not_panic() {
        assert!(HandshakeRespPkt::decode(&mut ReadCursor::new(&[0; 9])).is_err());
        assert!(HandshakeRespPkt::decode(&mut ReadCursor::new(&[0; 10])).is_ok());
    }
}
