...
impl TeachingState {
    ...
    pub fn observe(&mut self, id: Uuid, action: &InputAction, size: Option<(u16, u16)>) {
        self.teacher.observe(id, action, size);
    }
    pub fn observe_at(
        &mut self,
        id: Uuid,
...
impl AivanaApp {
    pub(super) fn poll_teaching(&mut self) {
        ...
        if let Some(id) = self.teaching.target {
            ...
            if !valid {
                ...
                self.teaching.teacher.notice =
                    "Wiedergabe zurückgesetzt: Ziel oder Bildgröße geändert".into();
            }
        }
    }
    pub(super) fn teaching_view(&mut self, ui: &mut Ui) {
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            if ui
                .add_enabled(!recording, egui::Button::new("Gespeicherten Ablauf laden"))
                .clicked()
            {
                match teaching::load() {
                    Ok(p) => {
        ...
        if ui
            ...
            .clicked()
        {
            self.status=match teaching::save(&self.teaching.teacher.procedure){Ok(())=>"Ablauf per Windows DPAPI gespeichert (ersetzt den bisherigen gespeicherten Ablauf)".into(),Err(e)=>format!("Ablauf speichern: {e:#}")};
        }
        ...
        if self.teaching.awaiting_evidence {
            ...
            if ui
                .button("Wirkung im Remote-Bild manuell bestätigt → nächster Schritt")
                .clicked()
            {
            ...
            .cloned()
        {
            ...
            if matches!(step, Step::TextRequired) {
            ...
            let enabled = !recording
                ...
                && (!matches!(step, Step::TextRequired) || !self.teaching.text.is_empty());
            if ui
                ...
                .clicked()
            {
                ...
                let result = (|| -> anyhow::Result<()> {
                    ...
                    let actions = self.teaching.teacher.procedure.actions(
                        &step,
                        size.unwrap(),
                        &self.teaching.text,
                    )?;
            ...
            ui.label("Alle Schritte einzeln ausgeführt und manuell bestätigt.");
        }
